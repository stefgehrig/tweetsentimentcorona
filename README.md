...
Two important caveats of the simple analysis of "virtual xenophonia" presented here are (i) the difficulty to establish causality and (ii) measurement error. The latter should be huge, given that natural language was transformed into unidimensional metrics - despite preivous validations of the methods, also in twitter data. Further, a tweet with negative sentiment is of course not necessarily xenophobic, but could simply contain words with negative valence because it expresses sadness. Thus, more advanced methods of text analysis (e.g., neural-network based hate speech detectors) and causal identification (e.g., synthetic controls, difference-in-difference) would be needed for thorough analysis.
